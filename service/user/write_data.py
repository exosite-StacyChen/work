...
def main(ExoSolution):
    host = "stacy3.apps.exosite-staging.io"
    solution = {
        "sid": "d1xp1r1vjkmbk0000"
    }
    # print "create role..."
...
def write(host, ExoSolution, solution, roleId, parameterName, start, end):
    data = ""
    ...
    for x in xrange(start, end):
        ...
        postData(host + "/assignUser",
                 {"id": x, "roles": [{"role_id": roleId, "parameters": parameters}]})
        single = single + "-----------------------------------------\n"
        single = single + "Count : {}\n".format(x)
        single = single + "name :{}  \nemail:{} \npassword:{} \nuserID:{} \nroleID: {} \nparameter_name:{}\n".format(
            name, email, password, x, roleId, parameterName)
        single = single + \
            "assignValue: name:{} value:{}\n".format(parameterName, "1-100")
        single = single + "-----------------------------------------\n"
        print single
        data = data + single
        time.sleep(1)
